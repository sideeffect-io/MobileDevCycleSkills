# Swift Reviewer handoff contract

<!-- swift-suite:REVIEW-HANDOFF-CONTRACT -->

Use this fallback only when an inter-agent lifecycle requires a transition. A repository-local
`SWIFT-HANDOFF/1` contract may add stricter requirements and takes precedence. A standalone review
...
| --- | --- | --- | --- |
| `SWIFT_REVIEWER` | `COMPLETE` | `APPROVED` | Acceptance, applicable gates, and required evidence pass with no unresolved required finding. |
| `SWIFT_REVIEWER` | `SWIFT_DEVELOPER` | `CHANGES_REQUIRED` | Implementation, test, or producible evidence findings require remediation. |
| `SWIFT_REVIEWER` | `SWIFT_ARCHITECT` | `CHANGES_REQUIRED` | Architecture findings require revised binding decisions before implementation. |
| `SWIFT_REVIEWER` | `ROOT` | `BLOCKED` | A user decision, authority, approval, or external state is required. |
...
FROM: SWIFT_REVIEWER
TO: COMPLETE | SWIFT_DEVELOPER | SWIFT_ARCHITECT | ROOT
