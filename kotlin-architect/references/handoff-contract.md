# Kotlin Architect handoff contract

<!-- kotlin-suite:ARCH-HANDOFF-CONTRACT -->

Use this fallback only when an inter-agent lifecycle requires a transition. A repository-local
`KOTLIN-HANDOFF/1` contract may add stricter requirements and takes precedence. Do not create a
...
| --- | --- | --- | --- |
| `KOTLIN_ARCHITECT` | `KOTLIN_DEVELOPER` | `READY` | Design is settled and implementable. |
| `KOTLIN_ARCHITECT` | `ROOT` | `BLOCKED` | A user decision, authority, approval, or external state is required. |
...
FROM: KOTLIN_ARCHITECT
TO: KOTLIN_DEVELOPER | ROOT
